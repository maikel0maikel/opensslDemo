...
void start_uclient(const char* remote_address,int remote_port,const char* uname,const char * upwd){
    ...
    start_p2p_client(remote_address, port, client_ifname, m_local_addr);
}

void start_bind_client(int type ,const char * address,int port){
    bind_client(type,address,port);
}